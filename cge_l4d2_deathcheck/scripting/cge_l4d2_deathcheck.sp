...
ConVar director_no_death_check, allow_all_bot_survivor_team;

ConVar g_hCvarAllow;

bool g_bCvarAllow, bLeftSafeRoom;
...
public void OnPluginEnd()
{
	ResetPlugin();
	ResetTimer();
	director_no_death_check.RestoreDefault();
	if(g_bL4D2Version) allow_all_bot_survivor_team.RestoreDefault();
}
...
void IsAllowed()
{
	...
	if( g_bCvarAllow == false && bCvarAllow == true)
	{
		...
		if(g_bL4D2Version) allow_all_bot_survivor_team.SetBool(true);
...
Action PlayerLeftStart(Handle timer)
{
	if (LeftStartArea() || bLeftSafeRoom)
	{
		...
		if (g_bL4D2Version)
		{
			allow_all_bot_survivor_team.SetBool(true);
		}
...
void Event_DeathCheck(Event event, const char[] name, bool dontBroadcast)
{  
