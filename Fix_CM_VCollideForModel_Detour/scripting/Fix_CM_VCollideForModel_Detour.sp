#define PLUGIN_VERSION		"1.2-2025/10/27"
...
#define GAMEDATA_CONF	"CM_VCollideForModel_Detour"
#define DEBUG			0
...
{
	name = "[L4D2] CM_VCollideForModel Crash Fix",
	author = "Dragokas",
	description = "Null pointer dereference fix in engine.so",
...
{
	g_Detour_Size = 6; // == sizeof(detour) + leftover of original replaced instruction size
	
	g_Addr_Func = hGameData.GetAddress("CM_VCollideForModel");
	if( !g_Addr_Func ) SetFailState("Failed to load \"%s\" address.", "CM_VCollideForModel");
...
void Detour_Linux()
{
	/*
		...
		85 D2 					test edx, edx
		0F 84 ? ? ? ? 			jz => retn
		8B 42 6C	 			mov eax, [edx+6Ch]  // bytes, replaced by detour
		83 F8 01				cmp eax, 1			// bytes, replaced by detour
		E9 ? ? ? ?				jmp => detour continue
	*/
	
	int iSize = 19; // sizeof( insertion )
...
void Detour_Windows()
{
	/*
		...
		0F 84 ? ? ? ? 			jz => retn
		8B 42 6C	 			mov eax, [edx+6Ch]  // bytes, replaced by detour
		83 E8 01				sub eax, 1			// bytes, replaced by detour
		E9 ? ? ? ?				jmp => detour continue
	*/
