...
// Defines how many special infected can be on the map on all gamemodes (does not count witch on all gamemodes)
...
// (L4D2) 1=Count special infected including tanks in all gamemode, 0=Don't count tank
// (L4D2) 1=最大特感數量會將在場的tank數量納入計算, 0=最大特感數量不要計算tank
"max_specials_tank_including"      "1"
...
// (L4D2) 1 = override tank limit in director vscript (Use carfully, this affects director tank, map tank, versus tank)
// (L4D2) 1 = 強制使用VScript覆蓋導演系統的Tank上限 (小心啟用, 會影響到導演生成的tank包括: 地圖固定tank, 對抗生成的tank)
"tank_limit_override"       "0"
